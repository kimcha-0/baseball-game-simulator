...
public class Field {
  ...
  private void clearBasepaths() {
    for (int i = 0; i < 4; i++) basepaths[i] = null;
  }
  ...
  private int updateOut(Player player) {
    ...
    if (error > field[hitDestination].fldPCT) {
      ...
      if (manOnThird()) runs += advanceFrom(3, 1);
      if (manOnSecond()) runs += advanceFrom(2, 1);
      if (manOnFirst()) runs += advanceFrom(1, 1);
  ...
  private int updateSingle(Player player) {
    ...
    if (manOnSecond()) {
      if (player.isFast()) {
        ...
        player.rbi++;
        field[0].er++;
      } else runs += advanceFrom(2, 1);
    }
    if (manOnFirst()) {
      if (!manOnThird() && player.isFast()) runs += advanceFrom(1, 2);
      else runs += advanceFrom(1, 1);
    }
    placeRunner(1, player);
    player.ab++;
    player.hit++;
  ...
  private void calcPitchCount() {
    field[0].pitchCount =
        ((4.81 * field[0].so)
            + (5.14 * field[0].bb)
            + (3.27 * (field[0].h + field[0].hbp))
            + (3.16 * (field[0].to - field[0].so)));
  }
  ...
  private String stateOfBasepaths() {
    String state = "";
    if (!manOnFirst() && !manOnSecond() && !manOnThird()) state = "Nobody on base.\n";
    else if (manOnFirst() && manOnSecond() && manOnThird()) state = "Man on 1B, 2B, and 3B\n";
    else if (manOnFirst() && !manOnSecond() && manOnThird()) state = "Man on 1B and 3B.\n";
    else if (manOnFirst() && manOnSecond() && !manOnThird()) state = "Man on 1B and 2B.\n";
    else if (manOnFirst() && !manOnSecond() && !manOnThird()) state = "Man on 1B.\n";
    else if (!manOnFirst() && manOnSecond() && !manOnThird()) state = "Man on 2B.\n";
    else if (!manOnFirst() && !manOnSecond() && manOnThird()) state = "Man on 3B.\n";
    else if (!manOnFirst() && manOnSecond() && manOnThird()) state = "Man on 2B and 3B.\n";
    return state;
  }
  ...
  private boolean manOnFirst() {
    return basepaths[1] != null;
  }

  private boolean manOnSecond() {
    return basepaths[2] != null;
  }

  private boolean manOnThird() {
    return basepaths[3] != null;
  }
