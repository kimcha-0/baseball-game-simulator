...
public class Game {
  ...
  public void playGame(
      Team init_home, Team init_away, League init_league, int[] homeInts, int[] awayInts)
      throws IOException {
    ...
    printLog(
        "-------------------------------------------------------------------------------------------\n");
    printLog(
        "Welcome to SimBaseball. Today's game is between the " + home + " and the " + away + "\n");
    printLog(
    ...
    while (aRuns == hRuns || inning < 10) {
      printLog(
          "Top of " + inning + ". " + home + ": " + hRuns + " " + away + ": " + aRuns + "\n\n");
      ...
      while ((outsBefore = diamond.getOuts()) < 3) {
        ...
        aRuns +=
            diamond.updateField(
                matchup(homeP, awayOrder[aSpot], calcBattingAdjustment(homeP, awayOrder[aSpot])),
                awayOrder[aSpot]);
        increment_aSpot();
        if (outsBefore < diamond.getOuts()) printLog("" + diamond.getOuts() + " outs!\n");
      }
      printLog(
          "Bottom of " + inning + ". " + home + ": " + hRuns + " " + away + ": " + aRuns + "\n\n");
      checkPitcherForSub(awayP);
      diamond.resetField(awayField);
      while ((outsBefore = diamond.getOuts()) < 3) {
        if (hRuns > aRuns && inning > 8) break;
        printLog(homeOrder[hSpot] + " is up to bat.\n");
        hRuns +=
            diamond.updateField(
                matchup(homeP, homeOrder[hSpot], calcBattingAdjustment(awayP, homeOrder[hSpot])),
                homeOrder[hSpot]);
        increment_hSpot();
        if (outsBefore < diamond.getOuts()) printLog("" + diamond.getOuts() + " outs!\n");
      }
  ...
  private double calcBattingAdjustment(Player pitcher, Player batter) {
    if (pitcher.LR.equals("R")) {
      if (batter.LR.equals("R")) return -.015;
      else return .030;
    } else {
      if (batter.LR.equals("L")) return -.090;
      else return .045;
    }
  }
  ...
  private int matchup(Player pitcher, Player player, double adjustment) {
    ...
    */
    ...
    if (gen <= aBBAVG) result = WALK;
    else if (gen <= aBBAVG + aSingleleAVG) result = SINGLE;
    else if (gen <= aBBAVG + aSingleleAVG + aDoubleAVG) result = DOUBLE;
    else if (gen <= aBBAVG + aSingleleAVG + aDoubleAVG + aTripleAVG) result = TRIPLE;
    else if (gen <= aBBAVG + aSingleleAVG + aDoubleAVG + aTripleAVG + aHRAVG) result = HOMERUN;
  ...
  private void addAwayField(Player p) {
    if (p.position.equals("P")) awayField[0] = p;
    else if (p.position.equals("C")) {
      if (awayField[1] != null) insertToEmptySlot(awayField, awayField[1], 1, 8);
      awayField[1] = p;
    } else if (p.position.equals("1B")) {
      if (awayField[2] != null) insertToEmptySlot(awayField, awayField[2], 1, 8);
      awayField[2] = p;
    } else if (p.position.equals("2B")) {
      if (awayField[3] != null) insertToEmptySlot(awayField, awayField[3], 1, 8);
      awayField[3] = p;
    } else if (p.position.equals("3B")) {
      if (awayField[4] != null) insertToEmptySlot(awayField, awayField[4], 1, 8);
      awayField[4] = p;
    } else if (p.position.equals("SS")) {
      if (awayField[5] != null) insertToEmptySlot(awayField, awayField[5], 1, 8);
      awayField[5] = p;
    } else if (p.position.equals("LF")) {
      if (awayField[6] != null) insertToEmptySlot(awayField, awayField[6], 1, 8);
      awayField[6] = p;
    } else if (p.position.equals("CF")) {
      if (awayField[7] != null) insertToEmptySlot(awayField, awayField[7], 1, 8);
      awayField[7] = p;
    } else if (p.position.equals("RF")) {
      if (awayField[8] != null) insertToEmptySlot(awayField, awayField[8], 1, 8);
      awayField[8] = p;
    } else if (p.position.equals("IF")) insertToEmptySlot(awayField, p, 2, 5);
    else if (p.position.equals("MI")) insertToEmptySlot(awayField, p, 3, 5);
    else if (p.position.equals("UT")) insertToEmptySlot(awayField, p, 1, 8);
    else if (p.position.equals("OF")) insertToEmptySlot(awayField, p, 6, 8);
  }
  ...
  private void addHomeField(Player p) {
    if (p.position.equals("P")) homeField[0] = p;
    else if (p.position.equals("C")) {
      if (homeField[1] != null) insertToEmptySlot(homeField, homeField[1], 1, 8);
      homeField[1] = p;
    } else if (p.position.equals("1B")) {
      if (homeField[2] != null) insertToEmptySlot(homeField, homeField[2], 1, 8);
      homeField[2] = p;
    } else if (p.position.equals("2B")) {
      if (homeField[3] != null) insertToEmptySlot(homeField, homeField[3], 1, 8);
      homeField[3] = p;
    } else if (p.position.equals("3B")) {
      if (homeField[4] != null) insertToEmptySlot(homeField, homeField[4], 1, 8);
      homeField[4] = p;
    } else if (p.position.equals("SS")) {
      if (homeField[5] != null) insertToEmptySlot(homeField, homeField[5], 1, 8);
      homeField[5] = p;
    } else if (p.position.equals("LF")) {
      if (homeField[6] != null) insertToEmptySlot(homeField, homeField[6], 1, 8);
      homeField[6] = p;
    } else if (p.position.equals("CF")) {
      if (awayField[7] != null) insertToEmptySlot(homeField, homeField[7], 1, 8);
      homeField[7] = p;
    } else if (p.position.equals("RF")) {
      if (homeField[8] != null) insertToEmptySlot(homeField, homeField[8], 1, 8);
      homeField[8] = p;
    } else if (p.position.equals("IF")) insertToEmptySlot(homeField, p, 2, 5);
    else if (p.position.equals("MI")) insertToEmptySlot(homeField, p, 3, 5);
    else if (p.position.equals("UT")) insertToEmptySlot(homeField, p, 1, 8);
    else if (p.position.equals("OF")) insertToEmptySlot(homeField, p, 6, 8);
  }

  private void printField(Player[] field) {
    for (Player p : field) System.out.println(p);
  }
