...
public class ScoreKeeper {
    ...
    homeRuns =
        awayRuns =
            homeHits =
                awayHits =
                    homeErrors =
                        awayErrors = homeAVGRuns = awayAVGRuns = homeAVGHits = awayAVGHits = 0;
    iterations =
        homeAVGErrors = awayAVGErrors = homeWins = awayWins = homeAVGWins = awayAVGWins = 0;
  }
  ...
  public void updateMultisimStats(Game game) {
    ...
    if (game.hRuns > game.aRuns) homeWins++;
    else awayWins++;
  ...
  private String teamTab(Team t) {
    ...
    int tabsNeeded;
    String tabs = "";
    if (length <= 15) tabsNeeded = 3;
    else if (length <= 25) tabsNeeded = 2;
    else tabsNeeded = 1;
    for (int i = 0; i < tabsNeeded; i++) tabs += "\t";
    return tabs;
  }
