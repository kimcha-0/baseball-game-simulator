// scrapes data from baseball-reference.com teampages
...
public class TeamBuilder {
  ...
  public Team buildTeam(String url) throws IOException {
    ...
    // go to beginning of Team batting and create entire team roster with batting stats
    ...
    for (; ; ) {
      nextLine = in.nextLine();
      if (!(nextLine.equals("<tr close=\" thead\">")) && !(nextLine.equals("</tbody>"))) {
        in.nextLine();
        in.nextLine();
        temp = findName();
    ...
    // go to beginning of Team fielding stats and set fielding stats for players
    in.findWithinHorizon("<tbody>", 0);
    in.nextLine();
    for (; ; ) {
      nextLine = in.nextLine();
  ...
  public void setBattingStats(Player p) {
    ...
    if (in.findInLine("(#)") != null) p.LR = "B";
    else if (in.findInLine("\\*") != null) p.LR = "L";
    else p.LR = "R";

    for (int i = 0; i < 2; i++) in.nextLine();
  ...
  public void setFieldingStats(Player p) {
    ...
    if ((value = in.findInLine("(\\d){1,4}")) == null) p.PO = 0;
    else p.PO = Double.valueOf(value);
    in.nextLine();
    if ((value = in.findInLine("(\\d){1,4}")) == null) p.ASS = 0;
    else p.ASS = Double.valueOf(value);
    in.nextLine();
    if ((value = in.findInLine("(\\d){1,4}")) == null) p.ERR = 0;
    else p.ERR = Double.valueOf(value);
  ...
  private String findName() {
    String name;
    if ((name = in.findInLine("[A-Z](\\w)*(\\s)[A-Z](\\w)*")) != null) return name;
    else return in.findInLine("([A-Z][.]){2}(\\s)(\\w)*");
  }
}
